...
export function drawSkillEvolution(c,{id,tier,rank=0,x,y,radius=70,sx=1,sy=1,time=0,alpha=1,reducedMotion=false}) {
  ...
  const colors={sanctuary:'#a3ffd4',orbital:'#ffc4ff',timestop:'#a9e8ff',bigbang:'#cf9dff',net:'#b3ffda',loach:'#8dffd2',frog:'#d3ff89',electric:'#adf2ff',flame:n>=3?'#a9d8ff':'#ffd391',freeze:'#b9f5ff',vortex:'#89eed8',chorus:'#e1ffa8',rewind:'#a1ddff',talisman:'#ffdb8a',palm:'#fff0ad',dragon:'#baffdc',blackhole:'#e2b5ff',meteor:'#ffc397',thunderstorm:'#c4c2ff'};
  ...
  if(rank>=4){c.save();c.rotate(phase*.25);c.strokeStyle=rank===5?'#e8ffff':'#ffe2a0';c.lineWidth=4;for(let j=0;j<rank-2;j++){c.rotate(Math.PI/(rank-2));c.beginPath();c.ellipse(0,0,r*1.15,r*.42,0,0,Math.PI*2);c.stroke();}c.restore();}
  if(rank){
...
export function drawEvolvedFlame(c,{rank,time,reducedMotion=false}) {
  const phase=reducedMotion?0:time, palette=rank===1?['#ff7b25','#ffe9a1']:rank===2?['#188fff','#a5ffff']:rank===3?['#a55aff','#fff0ff']:rank===4?['#ffb52c','#fff3bb']:['#71ffe9','#ffffff'];
  c.save();c.lineJoin='round';c.shadowColor=palette[0];c.shadowBlur=reducedMotion?0:18;
  const jets=Math.min(11,1+rank*2);
...
export function drawEvolvedBolt(c,{from,to,rank,sx,sy,time=0,reducedMotion=false}) {
  ...
  for(let i=1;i<=rank*2;i++){const p=points[Math.min(7,Math.max(1,Math.round(i/(rank*2)*7)))],sign=i%2?1:-1;c.beginPath();c.moveTo(p.x,p.y);c.lineTo(p.x+sign*24*sx,p.y-22*sy);c.lineTo(p.x+sign*15*sx,p.y-39*sy);c.lineTo(p.x+sign*42*sx,p.y-56*sy);c.strokeStyle=color;c.lineWidth=Math.max(1.5,rank*sx);c.stroke();}
